...
public class PlateformeCandidatures {
     ...
     * @return une sauvegarde représentant les couples d'associations obtenues à la fin de l'algorithme.
     */
    public Sauvegarde mariageStableEtudiantsBis()
    {
        while (!acceptationsTerminees())
        {
    ...
    public Sauvegarde mariageStableEtudiantsTer()
    {
        while (!acceptationsTerminees())
        {
            for (Etablissement etablissement : etablissements)
            {
                ...
                {
                    int nbPlaces = etablissement.nombrePlaces();
                    while ((nbPlaces != 0) && ((etablissement.getPreferences().size() - 1) >= etablissement.getCandidatEtudie()))
                    {
                        Etudiant candidatSouhaite = etablissement.getEtudiantSouhaite();
                        etablissement.incrementerCandidatEtudie();
                        if(!candidatSouhaite.estAffecte())
                        {
                            candidatSouhaite.setAffectation(etablissement);
                            etablissement.ajoutCandidat(candidatSouhaite);
                            nbPlaces--;
                        }
                        else if (candidatSouhaite.getListeVoeux().indexOf(candidatSouhaite.voeuActuel()) > candidatSouhaite.getListeVoeux().indexOf(etablissement) )
                        {
                            Etablissement aRejeter = candidatSouhaite.getEtablissementAffecte();
                            aRejeter.desaffecter(candidatSouhaite);
                            candidatSouhaite.setAffectation(etablissement);
                            etablissement.ajoutCandidat(candidatSouhaite);
                            nbPlaces--;
                        }
                    ...
                    // liste de X étudiants : candidats souhaités selons places
                    // on incrémente le candidats souhaités à qui on a enoyé
                    // pour tous les etdiants de cette liste
                        // l'établissement candidate : si pas affecté on l'affecte
                        // sinon l'étudiant regarde si ce nouvel etab est meilleur
                            // si oui : le prend comme voeu actuel
                            // si non : ?
                }
            }
        }
